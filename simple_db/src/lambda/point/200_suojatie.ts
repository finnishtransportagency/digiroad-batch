...
export const handler = async (event:any, ctx:any) => {
    ...
    try {
        ...
        {
            const sql = `
            drop table if exists simple.pedestrian_crossing;
            create table simple.pedestrian_crossing (
        ...
        const sql = `
        INSERT INTO simple.pedestrian_crossing (
            external_id,
            suggest_box,
            valid_from,
            valid_to,
