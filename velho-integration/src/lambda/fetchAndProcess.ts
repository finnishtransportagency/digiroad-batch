...
export const handler = async (event: { ely: string, asset_name: string, asset_type_id: number, asset_type: string, paths: string[] }, ctx: any) => {
    ...
    const index:AssetInLinkIndex=  {}
    addedWithDigiroadLinks.forEach(a => {
        a.linkData.forEach(a1=> {
            if (index[a1.linkId] == undefined || index[a1.linkId].length == 0)
                index[a1.linkId] = [{linkData:a1,asset:a.asset} as AssetInLink]
            else index[a1.linkId].push({linkData:a1,asset:a.asset} as AssetInLink)
        })
    })
    ...
    Object.keys(index).forEach(key => {
        if (index[key].length > 1) {
            ...
            sortedByLink.forEach(a => {
                    ...
                    const nextAsset = sortedByLink.find(a => link.mValueEnd==a.linkData.mValue  && link.mValue <a.linkData.mValue && a.asset.oid != p.oid  )
                    const beforeAsset = sortedByLink.find(a =>link.mValue ==  a.linkData.mValueEnd   && a.linkData.mValue > link.mValue  && a.asset.oid != p.oid)
                    const overlap = sortedByLink.find(a => a.linkData.mValue <=link.mValue ||  a.linkData.mValueEnd >=link.mValueEnd && a.asset.oid != p.oid )
